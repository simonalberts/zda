...
2011-07-17 v1.3.1.1 
         Improved handling by including trash
...
function get_my_revision() {
	...
	$sql = "SELECT `ID`,`post_date`,`post_title`,`post_modified`
			...
			WHERE `post_type` = 'revision' OR post_status = 'trash'
			ORDER BY `ID` DESC";
			echo $sql;
	$results = $wpdb -> get_results($sql);
	if($results) {
...
EOT;
	...
	_e('Great, not found wordpress redundant revision posts!<br />Today will be another beautiful day!','delete-revision');
	echo "</div></div>";}
}
...
function delete_revision_act() {
	...
	$sql = "DELETE FROM $wpdb->posts WHERE post_type = 'revision' OR post_status = 'trash'";
	$results = $wpdb -> get_results($sql);
